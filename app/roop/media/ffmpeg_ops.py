...
def transcode_video_range(
    ...
    writer_config = resolve_video_writer_config(
        roop.config.globals.video_encoder, roop.config.globals.video_quality
    )
...
def join_videos(
    videos: List[str], dest_filename: str, simple: bool, reencode: bool = False
):
    if simple:
...
def extract_frames(
    ...
    commands.extend(
        ["-vsync", "0", os.path.join(temp_directory_path, "%06d." + image_format)]
    )
    return run_ffmpeg(commands)
...
def create_video(
    ...
    run_ffmpeg(
        [
            ...
            os.path.join(
                temp_directory_path,
                f"%06d.{roop.config.globals.CFG.output_image_format}",
            ),
            "-c:v",
            roop.config.globals.video_encoder,
            "-crf",
            str(roop.config.globals.video_quality),
            "-pix_fmt",
            "yuv420p",
            "-vf",
...
def crop_media(
    ...
    return run_ffmpeg(
        ["-i", input_path, "-vf", crop_filter, "-c:a", "copy", output_path]
    )


def apply_media_transforms(
    input_path: str, output_path: str, vf_filters: list, is_video: bool
) -> bool:

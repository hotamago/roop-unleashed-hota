...
def render_status_markdown(state):
    ...
    memory_status = state.get("memory_status") or roop.config.globals.runtime_memory_status
    if memory_status:
...
def _apply_state(state, force_log=False):
    merged = _default_state()
    merged.update(state)
    roop.config.globals.runtime_processing_state = merged
    roop.config.globals.runtime_processing_status = render_status_line(merged)
    roop.config.globals.runtime_processing_markdown = render_status_markdown(merged)

    now = time.time()
    should_log = force_log
    if merged.get("status") in ("running", "stopping"):
        if now - roop.config.globals.runtime_processing_last_log_at >= 1.0:
            should_log = True
        ...
        if _is_number(total) and total > 0 and _is_number(completed) and completed >= total:
            should_log = True
    if should_log:
        print(f"[processing] {roop.config.globals.runtime_processing_status}", flush=True)
        roop.config.globals.runtime_processing_last_log_at = now
    return roop.config.globals.runtime_processing_markdown
...
def start_processing_status(message="Preparing faceswap job", total=None, unit="units", total_files=None, memory_status=None):
    ...
    state.update({
        ...
        "started_at": now,
        "elapsed": 0.0,
        "memory_status": memory_status or roop.config.globals.runtime_memory_status,
    })
...
def set_memory_status(memory_status):
    roop.config.globals.runtime_memory_status = memory_status
    state = _ensure_state()
    state["memory_status"] = memory_status
    return _apply_state(state, force_log=False)
...
def get_processing_status_markdown():
    if not roop.config.globals.runtime_processing_markdown:
        return render_status_markdown(_ensure_state())
    return roop.config.globals.runtime_processing_markdown


def get_processing_status_line():
    if not roop.config.globals.runtime_processing_status:
        return render_status_line(_ensure_state())
    return roop.config.globals.runtime_processing_status

...
import roop.utils.template_parser as template_parser
...
def sort_rename_frames(path: str):
    ...
    for i in range(len(filenames)):
        ...
        new_filename = os.path.join(
            path, f"{newidx:06d}." + roop.config.globals.CFG.output_image_format
        )
    ...
    return glob.glob(
        (
            os.path.join(
                ...
                f"*.{roop.config.globals.CFG.output_image_format}",
            )
        )
    )
...
def replace_template(file_path: str, index: int = 0) -> str:
    ...
    template = roop.config.globals.CFG.output_template
    replaced_filename = template_parser.parse(
        template, {"index": str(index), "file": fn, "timestamp": datetime.now().strftime('%Y%m%d%H%M%S')}
    )

    return os.path.join(roop.config.globals.output_path, f"{replaced_filename}{ext}")
...
def clean_temp(target_path: str) -> None:
    ...
    if not roop.config.globals.keep_frames and os.path.isdir(temp_directory_path):
...
def get_device() -> str:
    ...
    if len(roop.config.globals.execution_providers) < 1:
        if 'CUDAExecutionProvider' in available_providers:
            roop.config.globals.execution_providers = ['CUDAExecutionProvider']

...
from roop.media.capturer import get_image_frame, get_video_frame_total
...
class StagedBatchExecutor:
    ...
    def get_fallback_mgr(self):
        if self.fallback_mgr is None:
            ...
            self.fallback_mgr.initialize(roop.config.globals.INPUT_FACESETS, roop.config.globals.TARGET_FACES, self.options)
    ...
    def ensure_full_detect_stage(self, entry, endframe, detect_dir, stages, manifest, memory_plan):
        ...
        planner.initialize(roop.config.globals.INPUT_FACESETS, roop.config.globals.TARGET_FACES, self.build_stage_options([]))
        processed_frames = 0
        total_tasks = 0
        try:
    ...
    def ensure_full_swap_stage(self, entry, endframe, detect_dir, swap_dir, task_count, stages, manifest, memory_plan):
        ...
        swap_mgr.initialize(roop.config.globals.INPUT_FACESETS, roop.config.globals.TARGET_FACES, self.build_stage_options(["faceswap"]))
    ...
    def ensure_full_mask_stage(self, entry, endframe, detect_dir, swap_dir, mask_dir, task_count, stages, manifest, memory_plan):
        ...
        mask_mgr.initialize(roop.config.globals.INPUT_FACESETS, roop.config.globals.TARGET_FACES, self.build_stage_options([self.mask_name]))
    ...
    def ensure_full_enhance_stage(self, detect_dir, swap_dir, mask_dir, enhance_dir, task_count, stages, manifest, memory_plan):
        ...
        enhance_mgr.initialize(roop.config.globals.INPUT_FACESETS, roop.config.globals.TARGET_FACES, self.build_stage_options([self.enhancer_name]))
    ...
    def ensure_full_compose_stage(self, entry, endframe, fps, detect_dir, swap_dir, mask_dir, enhance_dir, intermediate_video, stages, manifest, memory_plan):
        ...
        compose_mgr.initialize(roop.config.globals.INPUT_FACESETS, roop.config.globals.TARGET_FACES, self.build_stage_options([]))
        fallback_mgr = None
            ...
            if compose_worker_count == 1:
                ...
                def get_thread_compose_mgr():
                    ...
                    if thread_compose_mgr is None:
                        ...
                        thread_compose_mgr.initialize(roop.config.globals.INPUT_FACESETS, roop.config.globals.TARGET_FACES, self.build_stage_options([]))

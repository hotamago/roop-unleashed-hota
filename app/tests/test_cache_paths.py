...
def test_jobs_root_uses_persistent_processing_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("roop.cache_paths.util.resolve_relative_path", fake_resolve)
...
def test_prepare_environment_separates_gradio_temp_from_runtime_temp(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(roop.globals, "CFG", SimpleNamespace(use_os_temp_folder=False), raising=False)
